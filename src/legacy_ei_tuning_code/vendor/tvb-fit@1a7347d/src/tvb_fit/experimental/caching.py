    ...
    * `redo`: if True, the computation is run even if the file already exists
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            global cache_path
            if not redo and os.path.exists(os.path.join(cache_path, f"{fname}.pkl")):
                print(f"Loading {fname} from cache")
                with open(os.path.join(cache_path, f"{fname}.pkl"), 'rb') as f:
                ...
                if caching:
                    ...
                    with open(os.path.join(cache_path, f"{fname}.pkl"), 'wb') as f:
                        pickle.dump(comp, f)
                # exec(f"global {fname}\n{fname} = comp")
            return comp
        return wrapper
